...
def worker_end_frame(input_image, end_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs,
    ...
    try:
        ...
        start_image_encoder_output = hf_clip_vision_encode(input_image_np, feature_extractor, image_encoder)
        ...
        for section_idx, latent_padding in enumerate(latent_paddings):
            ...
            # Use a weighted combination of start and end latents based on temporal position
            if has_end_image:
                # The latent_paddings go from end of video to start, so inverse the temporal position
